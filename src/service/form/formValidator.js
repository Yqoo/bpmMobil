...
FormValidator.doValidate = function(el, binding, vnode, value) {
  if (vnode.data.attrs.desc === "控件测试-单选框") {
  }

  //获取dom上的value，change事件直接拿value。keyup就算了
  if (value === undefined || el.type === "radio") {
...
function doValidate(conf) {
  ...
  else _valid = rule.rule(value, validValue);
...
FormValidator.format = function(msg, args) {
  //boolean类型的直接返回
  if (typeof args === "boolean") return msg;
...
FormValidator.rules = {
  ...
  phonenumber: {
    rule: function(v) {
      ...
      return /((\d{11})|^((\d{7,8})|(\d{4}|\d{3})-(\d{7,8})|(\d{4}|\d{3})-(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1})|(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1}))$)/i.test(
        v.trim()
      );
    },
    title: "手机号码",
